	...
	<view class="content">
		<view class="container" :style='{"minHeight":"100vh","width":"100%","padding":"0","position":"relative","background":"#F4F8FB","height":"auto"}'>
				...
				<view :style='{"padding":"0 30rpx","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="detail-list-item title">
					<view :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333"}' class="lable">公告标题：</view>
					<view :style='{"padding":"0px","margin":"0px","lineHeight":"80rpx","fontSize":"28rpx","color":"rgb(0, 0, 0)"}' class="text">{{detail.gonggaobiaoti}}</view>
				</view>
				...
				<view class="detail-list-item" :style='{"padding":"0 30rpx","margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"width":"auto","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333"}'>发布日期：</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"width":"100%","padding":"10rpx 24rpx","flexWrap":"wrap","background":"#f4f8fb","display":"flex","height":"auto"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"10rpx","color":"#fff","borderRadius":"8rpx","background":"#e29c95","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('gonggaoxinxi','私聊')" @tap="chatClick">联系TA</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"10rpx","color":"#fff","borderRadius":"8rpx","background":"#e29c95","width":"auto","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('gonggaoxinxi','私聊')" @tap="chatClick">联系TA</button>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discussgonggaoxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussgonggaoxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('gonggaoxinxi', this.id);
		...
		methods: {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('gonggaoxinxi',_this.detail)
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								name: _this.detail.gonggaobiaoti,
			...
			async upCallback(mescroll) {
				if(uni.getStorageSync("appUserid")){
					let res = await this.$api.list('discussgonggaoxinxi', {
						page: mescroll.num,
						limit: 10,
			...
			delClick(id){
				...
				uni.showModal({
					...
					async success(res) {
						if(res.confirm){
							await that.$api.del('discussgonggaoxinxi',JSON.stringify([id]))
							that.$utils.msg('删除成功')
							that.detail.discussnum--
							await that.$api.update('gonggaoxinxi',that.detail)
							setTimeout(()=>{
