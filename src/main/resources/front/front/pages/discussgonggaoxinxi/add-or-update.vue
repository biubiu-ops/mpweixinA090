	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussgonggaoxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussgonggaoxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussgonggaoxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussgonggaoxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussgonggaoxinxiCleanType',true);
