<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="公告标题" prop="gonggaobiaoti">
					<el-input v-model="ruleForm.gonggaobiaoti" placeholder="公告标题" clearable  :readonly="ro.gonggaobiaoti"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="公告标题" prop="gonggaobiaoti">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboot8n69wgx5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
