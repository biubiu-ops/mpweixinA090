	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="职称" prop="zhicheng">
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.mima!=''&&json.mima) || json.mima==0) && this.$storage.get("role")!="管理员"){
					...
					if(((json.zhicheng!=''&&json.zhicheng) || json.zhicheng==0) && this.$storage.get("role")!="管理员"){
